  ...
  return (
    <div className="flex h-full flex-col">
      <header className="shrink-0 border-b border-[var(--color-border-subtle)] bg-[var(--color-bg-base)] px-6 py-4">
        <h1 id="career-title" className="text-xl font-semibold text-[var(--color-text-primary)]">
          Career Center
        </h1>
      ...
      <main className="flex-1 overflow-y-auto p-6" aria-labelledby="career-title">
        <section aria-labelledby="jobs-heading" className="mb-8">
          <h2 id="jobs-heading" className="text-lg font-semibold text-[var(--color-text-primary)] mb-4">
            Opportunities
          </h2>
          {!hasJobs ? (
            <EmptyState
                ...
                <li key={job.id}>
                  <Card variant="default" padding="lg">
                    <div className="flex items-start justify-between">
                      <div>
                        <p className="font-medium text-[var(--color-text-primary)]">{job.position}</p>
        ...
        <section aria-labelledby="contacts-heading">
          <h2 id="contacts-heading" className="text-lg font-semibold text-[var(--color-text-primary)] mb-4">
            Network
          </h2>
          {!hasContacts ? (
            <EmptyState
              ...
              {contacts.map((c) => (
                <li key={c.id}>
                  <Card variant="default" padding="lg">
                    ...
                    <p className="text-sm text-[var(--color-text-secondary)]">{c.title} at {c.company}</p>
                    {c.sector ? (
                      <p className="text-xs text-[var(--color-text-muted)] mt-1">{c.sector}</p>
                    ) : null}
                  </Card>
                </li>
              ))}
